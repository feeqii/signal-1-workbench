...
const sample: InvestigationState = {
  schemaVersion: 1,
  caseId: "kras-k55",
  referenceHash: "a".repeat(64),
  title: "First question",
  question: "How does binding change?",
...
test("persists revisions across close/reopen and rejects lost updates atomically", async () => {
  ...
  assert.equal(
    typeof api.createStore,
    "function",
    "Durable investigation storage must be implemented",
  );
  ...
  try {
    ...
    const results = await Promise.allSettled([
      store.save(saved.id, 1, { ...sample, title: "A" }),
      store.save(saved.id, 1, { ...sample, title: "B" }),
    ]);
...
test("jobs are idempotent, recover expired leases and reject late completion after cancellation", async () => {
  ...
  assert.equal(
    typeof api.createStore,
    "function",
    "Durable job storage must be implemented",
  );
  ...
  try {
    ...
    const request = {
      ...
      inputRevision: 1,
      type: "comparison",
      input: { leftId: "a", rightId: "b", positions: [1, 2, 3] },
      inputHashes: ["x"],
    };
    ...
    assert.equal(
      await store.finish(first.id, claim.leaseToken, { answer: "old" }),
      false,
    );
    await store.cancel(first.id);
    assert.equal(
      await store.finish(first.id, recovered.leaseToken, { answer: "late" }),
      false,
    );
    ...
    assert.equal(
      await store.finish(active.id, active.leaseToken, { answer: "valid" }),
      true,
    );
    assert.equal(
      await store.finish(active.id, active.leaseToken, { answer: "duplicate" }),
      false,
    );
    assert.deepEqual((await store.getJob(first.id)).result, {
      answer: "valid",
    });
  } finally {
    await store.close();
...
test("restoring a bundle creates independent investigation and calculation identities atomically", async () => {
  ...
  try {
    assert.equal(
      typeof store.restore,
      "function",
      "Atomic bundle restoration must exist",
    );
