...
import {
  runScientificRequest,
  validateJobInput,
} from "@/lib/investigation/jobs";
...
export async function POST(request: Request) {
  try {
    const manifest = await loadCase(),
      bundle = verifyBundle(await readJson(request, 64_000_000), manifest);
    for (const job of bundle.jobs) {
      ...
      if (canonicalJson(verified) !== canonicalJson(job.result))
        throw new Error(
          "A saved calculation does not reproduce from its frozen inputs.",
        );
    }
    return Response.json(
      await (await getStore()).restore(bundle.investigation.state, bundle.jobs),
      { status: 201 },
    );
  } catch (error) {
