...
## Idle camera autosave loop correction

Reproduced the camera key-order mismatch through an actual in-memory PGlite JSONB round trip: the Mol* snapshot and database result have different JSON string order despite identical values. Replaced Workbench's string equality with recursive value equality that ignores object-field ordering while preserving vector ordering and numerical differences. The browser helper imports no Node modules. Camera restoration also checks this equality before calling Mol* `setState`, preventing unchanged normalized snapshots from generating another state notification.

Two regressions were observed failing first: JSONB semantic equality and redundant state application to the actual Mol* Camera. Both pass after the correction, including a changed vector coordinate that must remain unequal. All **21 targeted UI/client tests pass**; full TypeScript and scoped ESLint pass. Final idle-revision stability still requires controller production-browser verification after rebuilding.
